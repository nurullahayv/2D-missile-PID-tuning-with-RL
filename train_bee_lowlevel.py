"""
Training script for low-level bee colony policy.
Low-level policy learns movement and building actions.
"""
import os
import time
import shutil
...
from envs.env_bee_lowlevel import BeeLowLevelEnvRLlib
from models.ac_models_bee import BeeLowLevelModel
...
class BeeColonyCallbacks(DefaultCallbacks):
    """Custom callbacks for logging bee colony metrics."""
    ...
    def on_episode_end(self, *, worker, base_env, policies, episode, **kwargs):
        ...
        if info and '__common__' in info:
            ...
            episode.custom_metrics["active_builds"] = common_info.get('active_builds', 0)


def main():
    # Load config
    config_obj = BeeColonyConfig(mode=0)  # Mode 0 = low-level training
    args = config_obj.get_arguments

    print(f"\n{'='*60}")
    print(f"Bee Colony Low-Level Training")
    print(f"{'='*60}")
    ...
    # Register model
    ModelCatalog.register_custom_model("bee_lowlevel_model", BeeLowLevelModel)
    ...
    eval_env = BeeLowLevelEnvRLlib(args.env_config)
